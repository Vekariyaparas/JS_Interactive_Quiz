...
//If option select and icon adjacement
function optionSeleted(answer) {
    ...
    // once user selected disabled options 
    for (let i = 0; i < allOptions; i++) {
...
//If question completed and show the result
function showResultBox() {
    ...
    if (userScore > 7) {
        let scoreTag = '<span>ans congrats! You got <p>' + userScore + '</p> out of <p>' + questions.length + '</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else if (userScore > 3) {
        let scoreTag = '<span>ans nice, You got only <p>' + userScore + '</p> out of <p>' + questions.length + '</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else {
        let scoreTag = '<span>ans sorrey, You got only <p>' + userScore + '</p> out of <p>' + questions.length + '</p></span>';
        scoreText.innerHTML = scoreTag;
    }
}

//If set the time in particular question
...
function startTimerLine(time) {
    counterLine = setInterval(timer, 35.5);
    function timer() {
        time += 1;
